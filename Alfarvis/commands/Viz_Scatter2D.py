...
class VizScatter2D(AbstractCommand):
    ...
    def evaluate(self, array_datas):
        ...
        result_object = VizContainer.createResult(win, array_datas, ['scatter2d'])
        result_object.data = [win, properties, [array, row_colors,kl1], self.updateFigure]
    ...
    def updateFigure(self, result_data):
        ...
        if properties["invert"]:
            tempVar = np.copy(array[:,0])
            array[:,0] = np.copy(array[:,1])
            array[:,1] = np.copy(tempVar)
            tempVar2 = kl1[0]
            kl1[0] = kl1[1]
            kl1[1] = tempVar2
